...
import { executeMiniproyecto, submitMiniproyecto } from '../utils/submitMiniproyecto';
...
function getInitialTemplate(config?: MiniproyectoConfig | null) {
  return stripMarkdownCodeFences(config?.metodo?.plantilla || '') || DEFAULT_TEMPLATE;
}
...
export function ProgrammingMiniproyectoView({ content, onBack }: ProgrammingMiniproyectoViewProps) {
  ...
  const wrapperClassName = 'overflow-hidden rounded-[2rem] bg-[#F2F2F2]';
  const workspaceClassName = 'grid grid-cols-1 gap-5 p-4 lg:min-h-[calc(100vh-8.5rem)] lg:p-6';
  ...
  const guideItems = [
    ...
    'Revisa los casos visibles antes de enviar para entender como se validara tu solucion.',
    'Usa Ejecutar para probar y Enviar para calificar tu miniproyecto.'
  ];
  ...
  const [isLoadingMiniproyecto, setIsLoadingMiniproyecto] = useState(true);
  ...
  const [lenguajeSeleccionado, setLenguajeSeleccionado] = useState<number>(62);

  const methodPreview = buildMethodPreview(config?.metodo);
  const configuredCases = Array.isArray(config?.casos_prueba) ? config.casos_prueba : [];
      ...
      try {
        ...
        if (data?.respuesta_miniproyecto) {
          try {
            ...
            if (parsed?.tipo === 'programacion' || parsed?.esperado || parsed?.casos_prueba || parsed?.metodo) {
              setConfig(parsed as MiniproyectoConfig);
              setCode(getInitialTemplate(parsed as MiniproyectoConfig));
        ...
        setIsLoadingMiniproyecto(false);
      }
    };
  ...
  const handleClear = () => {
    setCode(getInitialTemplate(config));
    clearResults();
  };
  ...
  const handleExecute = async () => {
    ...
    const result = await executeMiniproyecto(content.id, { codigo: code, lenguaje_id: lenguajeSeleccionado });
  ...
  const handleSubmit = async () => {
    ...
    const result = await submitMiniproyecto(content.id, { codigo: code, lenguaje_id: lenguajeSeleccionado }, estudianteId);
    const data: any = result.data || {};

    if (result.status === 409) {
      setAprobado(true);
      setFeedback(data?.retroalimentacion || result.message || 'Ya tienes este miniproyecto aprobado.');
      if (Array.isArray(data?.casosPrueba)) setCasosPruebaResultados(data.casosPrueba);
      toast.info('Miniproyecto ya aprobado', { description: result.message || 'Ya tienes este miniproyecto aprobado.' });
      setIsSubmitting(false);
      return;
    }
    ...
    if (result.status === 400) {
      setFeedback(data?.retroalimentacion || data?.resumen || data?.estado || 'La solucion no supero la validacion.');
      if (typeof data?.puntosObtenidos === 'number') setPuntos(data.puntosObtenidos);
      setAprobado(false);
      toast.error(data?.resultado || 'No cumple', { description: data?.resumen || data?.message || 'La solucion no supero la validacion.' });
      setIsSubmitting(false);
      return;
    }
    ...
    <div className={wrapperClassName}>
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex min-w-0 items-center gap-4">
              ...
              <div className="min-w-0">
                <h1 className="truncate font-bold text-[#3A4A5B]">Miniproyecto de Programacion</h1>
      ...
      <div className={workspaceClassName}>
        ...
        <section className={`${cardClass} relative overflow-hidden`}>
          ...
          <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_auto] md:items-start">
            <div className="min-w-0">
              <div className="mb-3 border-b border-slate-100 pb-3">
                <span className={sectionLabelClass}>Miniproyecto</span>
                <h2 className="max-w-[32rem] text-[1.55rem] font-bold leading-tight text-[#3A4A5B]">{content.title}</h2>
              </div>
            </div>
          ...
          <p className="mt-4 border-t border-slate-100 pt-4 text-[14px] leading-6 text-gray-600">Compila, ejecuta y envia tu solucion con la misma dinamica del ejercicio compilador, manteniendo el contexto de miniproyecto.</p>
        </section>
        ...
        <section className="grid grid-cols-2 gap-5">
          <div className={`${cardClass} relative flex min-w-0 flex-col overflow-hidden`}>
            ...
            {descripcion ? (
              <div className="mb-4 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm leading-6 text-gray-700" dangerouslySetInnerHTML={{ __html: descripcion }} />
            ) : null}
        ...
        <section className={`${cardClass} lg:max-h-[34vh] lg:overflow-y-auto`}>
          ...
          {configuredCases.length === 0 ? (
            <p className="text-sm text-gray-500">No hay casos de prueba visibles para este miniproyecto.</p>
          ) : (
            <div className="flex gap-4 overflow-x-auto pb-2">
              {configuredCases.map((item, index) => {
                const accent = getCaseAccent();
                return (
                  <div key={`${index}-${item.inputs || ''}`} className={`min-w-[300px] flex-1 rounded-[1.25rem] border p-4 ${accent.card}`}>
                    <div className="mb-4 flex items-center justify-between gap-3">
                      <div className={`rounded-full px-3 py-1 text-xs font-semibold ${accent.badge}`}>Caso {index + 1}</div>
                      <div className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: accent.dot }} />
                    </div>
        ...
        <section className="w-full min-w-0 overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-[0_16px_36px_rgba(58,74,91,0.10)] lg:flex lg:min-h-0 lg:flex-col">
          <div className="flex flex-col gap-4 border-b border-slate-200 bg-[linear-gradient(90deg,rgba(74,144,226,0.07)_0%,rgba(255,255,255,1)_42%,rgba(126,214,167,0.07)_100%)] px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <button
                ...
                disabled={isRunning || isSubmitting || isLoadingMiniproyecto}
              ...
              <button
                ...
                disabled={isSubmitting || isRunning || aprobado || isLoadingMiniproyecto}
                ...
                style={{ background: isSubmitting || isRunning || aprobado || isLoadingMiniproyecto ? '#94a3b8' : 'linear-gradient(135deg, #4A90E2 0%, #5B9FED 55%, #7ED6A7 100%)' }}
              >
          ...
          <div className="flex flex-col lg:min-h-0 lg:flex-1">
            ...
            <div className="border-t border-slate-200 bg-white p-5 lg:max-h-[260px] lg:overflow-y-auto">
              ...
              {isLoadingMiniproyecto ? (
                <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50 px-4 py-6 text-sm text-slate-500">Cargando miniproyecto...</div>
