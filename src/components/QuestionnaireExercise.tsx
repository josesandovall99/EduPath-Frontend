    ...
    <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-md">
        ...
        <div className="space-y-6">
          {preguntas.map((pregunta, index) => (
            <div key={pregunta.id} className="rounded-xl border border-gray-200 p-4">
              <div className="mb-3 text-sm font-semibold text-[#3A4A5B]">
                ...
                <div className="space-y-2">
                  {(pregunta.opciones || []).map((option, optionIndex) => (
                    <button
                      ...
                      style={{
                        borderColor: answers[pregunta.id] === option ? '#4A90E2' : '#E5E7EB',
                        backgroundColor: answers[pregunta.id] === option ? '#EAF3FF' : 'white'
                      }}
                    >
                      {option}
                    </button>
      ...
      {configurableMode ? (
        ...
        <div className="mt-6 flex items-center justify-center gap-3">
          <button onClick={handlePreview} disabled={!canSubmit} className="rounded-lg border px-6 py-2 disabled:opacity-50">
            Preview
          </button>
          <button
            onClick={handleSubmit}
            disabled={!canSubmit || isSubmitting || aprobado}
            className="rounded-lg px-6 py-2 text-white"
            style={{ backgroundColor: (!canSubmit || isSubmitting || aprobado) ? '#9CA3AF' : '#4A90E2' }}
          >
