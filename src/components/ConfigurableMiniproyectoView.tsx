...
function normalizeExerciseType(exercise: EmbeddedExercise | LegacyExerciseResponse) {
  if (exercise.tipo_ejercicio !== 'Preguntas') {
    return exercise.tipo_ejercicio;
  }
...
export function ConfigurableMiniproyectoView({ content, onBack }: ConfigurableMiniproyectoViewProps) {
  ...
  const loadConfigurableProgress = async (miniproyectoId: string | number) => {
    try {
      ...
      const nextMap = Object.fromEntries(
        Array.isArray(progressData?.ejercicios)
          ? progressData.ejercicios.map((item) => [String(item.id), Boolean(item.esCorrecta ?? item.aprobado)])
          : []
      );
  ...
  useEffect(() => {
    ...
    const loadConfigurableMiniproyecto = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error('No se pudo cargar el miniproyecto.');
        }
        ...
        if (!parsedPayload) {
          throw new Error('Este miniproyecto no está configurado como paquete de ejercicios.');
        }
        ...
        if (parsedPayload.exercises.length > 0) {
          ...
          fetchedLegacyExercises = await Promise.all(
            (parsedPayload.exerciseIds || []).map(async (exerciseId) => {
              const exerciseResponse = await fetch(`${API_BASE_URL}/ejercicios/${exerciseId}`);
              if (!exerciseResponse.ok) {
                throw new Error(`No se pudo cargar el ejercicio ${exerciseId}.`);
              }
        ...
        if (cancelled) return;
        console.error('Error cargando miniproyecto configurable:', loadError);
  ...
  const updateExerciseResponse = (exerciseId: string, response: any) => {
    setExerciseResponses((prev) => {
      ...
      if (serializeConfigurableResponse(currentResponse) === serializeConfigurableResponse(response)) {
        return prev;
      }
  ...
  const handleEvaluateMiniproyecto = async () => {
    ...
    try {
      ...
      if (response.ok) {
        alert(data?.message || 'El miniproyecto fue evaluado correctamente.');
        return;
      }

      alert(data?.message || 'No fue posible evaluar el miniproyecto configurable.');
    } catch (evaluationError) {
      console.error('Error evaluando miniproyecto configurable:', evaluationError);
  ...
  const renderExerciseCard = (selectedExercise: EmbeddedExercise, index: number) => {
    ...
    let exerciseContent: JSX.Element;

    if (normalizedType === 'Compilador') {
      exerciseContent = (
        <ProgrammingContentView
          key={`configurable-${selectedExercise.id}`}
          content={contentProps}
          onBack={() => undefined}
          embedded
          configurableMode
          configurableResponse={exerciseResponses[String(selectedExercise.id)]}
          onConfigurableResponseChange={(response) => updateExerciseResponse(String(selectedExercise.id), response)}
          exerciseData={{
            id: Number(selectedExercise.id),
            contenido_id: 0,
            puntos: selectedExercise.puntos || 100,
            resultado_ejercicio: selectedExercise.resultado_ejercicio || '',
            codigoEstructura: selectedExercise.codigoEstructura || undefined,
            tipo_ejercicio: selectedExercise.tipo_ejercicio,
      ...
      exerciseContent = (
        <MultipleChoiceExercise
          ...
          enunciado={selectedExercise.configuracion?.enunciado}
          opciones={selectedExercise.configuracion?.opciones}
          onBack={() => undefined}
          embedded
          configurableMode
          configurableResponse={exerciseResponses[String(selectedExercise.id)]}
      ...
      exerciseContent = (
        <OrderingExercise
          ...
          activity={{ id: String(selectedExercise.id), title: exerciseTitle }}
          enunciado={selectedExercise.configuracion?.enunciado}
          items={selectedExercise.configuracion?.items}
          onBack={() => undefined}
          embedded
          configurableMode
          configurableResponse={exerciseResponses[String(selectedExercise.id)]}
      ...
      exerciseContent = (
        <MatchingExercise
          ...
          activity={{ id: String(selectedExercise.id), title: exerciseTitle }}
          enunciado={selectedExercise.configuracion?.enunciado}
          pares={selectedExercise.configuracion?.pares}
          onBack={() => undefined}
          embedded
          configurableMode
          configurableResponse={exerciseResponses[String(selectedExercise.id)]}
      ...
      exerciseContent = (
        <QuestionnaireExercise
          key={`configurable-${selectedExercise.id}`}
          activity={{ id: String(selectedExercise.id), title: exerciseTitle }}
          preguntas={Array.isArray(selectedExercise.configuracion?.preguntas) ? selectedExercise.configuracion?.preguntas : []}
          onBack={() => undefined}
          embedded
          configurableMode
          configurableResponse={exerciseResponses[String(selectedExercise.id)]}
      ...
      exerciseContent = (
        <div className="rounded-2xl border border-amber-200 bg-amber-50 px-6 py-8 text-sm text-amber-800">
          El tipo de ejercicio {normalizedType} todavía no está habilitado dentro del miniproyecto configurable.
        </div>
      );
    }
    ...
    return (
      <article
        ...
        className={`overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-[0_14px_34px_rgba(58,74,91,0.08)] ${isLargeExercise ? 'md:col-span-2' : ''}`}
      >
        <div className="border-b border-slate-100 px-6 py-5">
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div>
              <div className="inline-flex rounded-full bg-[#EAF2FF] px-3 py-1 text-xs font-semibold text-[#3978E8]">
                Ejercicio {index + 1}
              </div>
              ...
              {selectedExercise.descripcion ? (
                <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-500">{selectedExercise.descripcion}</p>
              ) : null}
            </div>
            <div className="flex flex-wrap items-center gap-2 md:justify-end">
              ...
              <span className="inline-flex rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600">
                {selectedExercise.puntos || 100} pts
              </span>
            </div>
          </div>
        </div>
      ...
      <header className="border-b border-slate-200 bg-white/95 shadow-[0_8px_24px_rgba(15,23,42,0.06)] backdrop-blur-sm">
        <div className="mx-auto flex max-w-[900px] items-center justify-between gap-4 px-4 py-3 lg:px-5">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="flex h-32px w-32px shrink-0 items-center justify-center rounded-xl bg-[linear-gradient(135deg,#14B8E6_0%,#2563EB_100%)] text-sm font-semibold text-white" style={{ width: '32px', height: '32px' }}>
              MP
            </div>
      ...
      <main className="mx-auto max-w-[900px] px-4 py-6 lg:px-5 lg:py-6">
        ...
        {isLoading ? (
          <div className="rounded-2xl border border-gray-200 bg-white px-6 py-12 text-center text-gray-500 shadow-sm">
            Cargando miniproyecto configurable...
          </div>
        ) : error ? (
          ...
          <div className="space-y-7">
            <section className="pt-3">
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                <article className="rounded-[22px] bg-[linear-gradient(135deg,#3B82F6_0%,#2563EB_100%)] px-6 py-5 text-white shadow-[0_18px_34px_rgba(37,99,235,0.28)]">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <div className="text-sm font-medium text-white/80">Ejercicios</div>
                      <div className="mt-2 text-[2.15rem] font-semibold leading-none">{totalExercises}</div>
                    </div>
                ...
                <article className="rounded-[22px] bg-[linear-gradient(135deg,#22C55E_0%,#05B34B_100%)] px-6 py-5 text-white shadow-[0_18px_34px_rgba(5,179,75,0.25)]">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <div className="text-sm font-medium text-white/80">Completados</div>
                      <div className="mt-2 text-[2.15rem] font-semibold leading-none">{correctExercises}/{totalExercises}</div>
                    </div>
                    <div className="rounded-2xl bg-white/16 p-3">
                ...
                <article className="rounded-[22px] bg-[linear-gradient(135deg,#C026D3_0%,#7C3AED_100%)] px-6 py-5 text-white shadow-[0_18px_34px_rgba(124,58,237,0.24)]">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <div className="text-sm font-medium text-white/80">Precisión</div>
                      <div className="mt-2 text-[2.15rem] font-semibold leading-none">{accuracy}%</div>
                    </div>
                    <div className="rounded-2xl bg-white/16 p-3">
            ...
            <section className="rounded-[24px] border border-[#C9E2FF] bg-[linear-gradient(180deg,#EEF7FF_0%,#F2FBFF_100%)] px-5 py-6 shadow-[0_16px_36px_rgba(80,140,220,0.12)] md:px-5 md:py-6">
              ...
              <div className="mt-6 grid gap-4 md:grid-cols-[minmax(0,1.45fr)_minmax(240px,0.75fr)]">
                <div className="rounded-[22px] bg-white px-5 py-5 shadow-sm ring-1 ring-slate-100">
                  ...
                  {hasRenderableDescription(miniproyectoDescripcion) ? (
                    <div
                      className="prose prose-sm mt-4 max-w-none text-slate-600 prose-headings:text-[#243447] prose-p:text-slate-600 prose-li:text-slate-600"
                      dangerouslySetInnerHTML={{ __html: miniproyectoDescripcion }}
                    />
                  ) : (
                    <p className="mt-4 text-sm leading-6 text-slate-500">
                      Este miniproyecto no tiene una descripción adicional configurada.
                    </p>
                  )}
                </div>

                <div className="space-y-4">
                  <div className="rounded-[22px] bg-white px-5 py-4 shadow-sm ring-1 ring-slate-100">
                    <div className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-400">Nivel</div>
                    <div className="mt-2 text-base font-semibold text-[#243447]">{miniproyectoNivel || 'No definido'}</div>
                  </div>
                  <div className="rounded-[22px] bg-white px-5 py-4 shadow-sm ring-1 ring-slate-100">
                    <div className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-400">Entregable esperado</div>
                    <div className="mt-2 text-sm leading-6 text-slate-600">{miniproyectoEntregable || 'No se definió un entregable para este miniproyecto.'}</div>
                  </div>
                </div>
              </div>
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-3">
                  <div className="flex items-center gap-3 rounded-2xl px-2 py-1">
                    ...
                    <div>
                      ...
                      <div className="text-[1.65rem] font-semibold leading-none text-emerald-600">{correctExercises}/{totalExercises}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 rounded-2xl px-2 py-1">
                    <Circle className="h-5 w-5 text-[#4A90E2]" />
                    <div>
                      <div className="text-xs text-slate-400">Respondidas</div>
                      <div className="text-[1.65rem] font-semibold leading-none text-[#3779F1]">{answeredExercises}/{totalExercises}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 rounded-2xl px-2 py-1">
                    ...
                    <div>
                      ...
                      <div className="text-[1.65rem] font-semibold leading-none text-rose-500">{incorrectExercises}</div>
                    </div>
                  </div>
                </div>
              </div>

              <div className="mt-4 rounded-2xl bg-[linear-gradient(135deg,#3B82F6_0%,#2F74E8_100%)] px-4 py-3 text-sm font-medium text-white shadow-sm">
                Nota: El miniproyecto se evaluará en un solo resultado final. {readyToEvaluate ? 'Ya puedes usar el botón principal de evaluación.' : `Todavía faltan ${Math.max(totalExercises - answeredExercises, 0)} ejercicios por responder.`}
              </div>
            </section>

            {showChatbot ? (
              <section className="space-y-4">
                <div className="rounded-[24px] border border-sky-100 bg-[linear-gradient(135deg,#F2F8FF_0%,#F8FCFF_100%)] px-5 py-4 text-sm leading-6 text-slate-600 shadow-sm">
                  <span className="font-semibold text-[#1F3A5F]">Apóyate del chatbot</span> para entender el contexto del caso, aclarar dudas del cliente simulado y resolver mejor los ejercicios del miniproyecto.
                </div>
                ...
                <MiniproyectoChatbotPanel
                  ...
                  areaId={miniproyecto?.Area?.id || content.areaId || null}
                  miniproyectoId={miniproyecto?.id || content.id}
                  title="Petitbot"
                  subtitle="Chatbot configurado para este miniproyecto"
                  contextLabel={miniproyecto?.Actividad?.titulo || content.title}
                />
                </div>
              </section>
            ) : null}
            ...
            <section className="rounded-[24px] border border-slate-200 bg-white px-6 py-6 shadow-[0_16px_34px_rgba(58,74,91,0.08)]">
              <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center rounded-full border-2 border-[#3779F1] px-5 py-2.5 text-sm font-semibold text-[#3779F1] transition-colors hover:bg-[#EFF5FF] disabled:cursor-not-allowed disabled:border-slate-300 disabled:text-slate-400"
                >
              ...
              <div className="mt-8">
                <div className="mb-2 flex items-center justify-between gap-3 text-sm text-slate-500">
                  <span>Progreso</span>
                  <span className="font-semibold text-[#111827]">{correctExercises}/{totalExercises} completado</span>
                </div>
                <div className="h-3 overflow-hidden rounded-full bg-slate-200">
                  <div
                    className="h-full rounded-full bg-[linear-gradient(90deg,#0F172A_0%,#111827_45%,#334155_100%)] transition-all duration-300"
