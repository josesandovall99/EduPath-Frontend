...
export function MiniproyectoManagementScreen({
  ...
  const [selectedAsignaturaFilterId, setSelectedAsignaturaFilterId] = useState<string>(docenteAsignaturaId ? String(docenteAsignaturaId) : '');
  ...
  // El tipo de editor depende del Asignatura del miniproyecto, no del id de la actividad.
  const selectedAsignaturaName = normalizeasignaturaName(selected?.Asignatura?.nombre);
  const isProgrammingMiniproyecto = selectedAsignaturaName.includes('programacion');
  const isManagementMiniproyecto = selectedAsignaturaName.includes('alcance') || selectedAsignaturaName.includes('gestion');
  ...
  const loadSupportingData = async () => {
    try {
      ...
      const [asignaturasResponse, chatbotsResponse] = await Promise.all([
        apiFetch(asignaturasEndpoint),
        apiFetch('/chatbots'),
      ]);

      if (!asignaturasResponse.ok) {
        throw new Error('No se pudieron cargar las asignaturas disponibles');
      }
      ...
      const [asignaturasData, chatbotsData] = await Promise.all([
  ...
  const loadMiniproyectos = async () => {
    ...
    setError(null);
    try {
      const response = await apiFetch('/miniproyectos', {}, docenteAsignaturaId ?? null);
      if (!response.ok) {
  ...
  const filteredMiniproyectos = useMemo(() => {
    ...
    const asignaturaIdFilter = Number(selectedAsignaturaFilterId);

    return miniproyectos.filter((item) => {
      const titulo = item.Actividad?.titulo?.toLowerCase() || '';
      const Asignatura = item.Asignatura?.nombre?.toLowerCase() || '';
      const nivel = item.Actividad?.nivel_dificultad?.toLowerCase() || '';
      const itemasignaturaId = Number(item.Asignatura?.id);
      const matchesAsignatura = !Number.isInteger(asignaturaIdFilter) || asignaturaIdFilter <= 0 || itemasignaturaId === asignaturaIdFilter;
      const matchesQuery = !term || titulo.includes(term) || Asignatura.includes(term) || nivel.includes(term);

      return matchesAsignatura && matchesQuery;
    });
  }, [miniproyectos, query, selectedAsignaturaFilterId]);

  const availableAsignaturas = useMemo(() => {
    return [...asignaturas].sort((left, right) => left.nombre.localeCompare(right.nombre));
  }, [asignaturas]);

  useEffect(() => {
    if (!selected) return;

    const selectedItemasignaturaId = Number(selected.Asignatura?.id);
    const asignaturaIdFilter = Number(selectedAsignaturaFilterId);

    if (Number.isInteger(asignaturaIdFilter) && asignaturaIdFilter > 0 && selectedItemasignaturaId !== asignaturaIdFilter) {
      clearSelectedEditor();
    }
  }, [selected, selectedAsignaturaFilterId]);

  const selectedCreateasignaturaId = Number(createForm.asignaturaId);

  const createEligibleChatbots = useMemo(() => {
    if (!Number.isInteger(selectedCreateasignaturaId) || selectedCreateasignaturaId <= 0) return [];

    return chatbots
      .filter((chatbot) => (
        chatbot.estado !== false &&
        (chatbot.miniproyecto_id === null || chatbot.miniproyecto_id === undefined) &&
        Number(chatbot.asignatura_id) === selectedCreateasignaturaId
      ))
      .sort((left, right) => left.nombre.localeCompare(right.nombre));
  }, [chatbots, selectedCreateasignaturaId]);

  const selectedEligibleChatbots = useMemo(() => {
    const selectedAsignaturaId = Number(selected?.Asignatura?.id);
    if (!isConfigurableMiniproyecto || !Number.isInteger(selectedAsignaturaId) || selectedAsignaturaId <= 0) return [];

    return chatbots
      .filter((chatbot) => (
        chatbot.estado !== false &&
        (chatbot.miniproyecto_id === null || chatbot.miniproyecto_id === undefined || Number(chatbot.miniproyecto_id) === Number(selected?.id)) &&
        Number(chatbot.asignatura_id) === selectedAsignaturaId
      ))
      .sort((left, right) => left.nombre.localeCompare(right.nombre));
  }, [chatbots, selected?.Asignatura?.id, selected?.id, isConfigurableMiniproyecto]);
  ...
  const groupedMiniproyectos = useMemo(() => {
    const groups = new Map<number, { Asignatura: AsignaturaInfo | null; items: MiniproyectoItem[] }>();

    filteredMiniproyectos.forEach((item) => {
      const asignaturaId = Number(item.Asignatura?.id) || 0;
      const current = groups.get(asignaturaId);
      if (current) {
        current.items.push(item);
        return;
      }

      groups.set(asignaturaId, {
        Asignatura: item.Asignatura || null,
        items: [item],
      });
    });

    return Array.from(groups.values()).sort((left, right) => {
      const leftName = left.Asignatura?.nombre || 'Sin asignatura';
      const rightName = right.Asignatura?.nombre || 'Sin asignatura';
  ...
  const handleSelect = (item: MiniproyectoItem) => {
      ...
      try {
        ...
        parsedSchedule = cronogramaRaw.map((entry: any) => {
          if (entry && typeof entry === 'object') {
            return {
              milestone: (entry.milestone ?? entry.hito ?? entry.activity ?? entry.actividad ?? entry.tAsignatura ?? '').toString(),
    ...
    setAssumptionInput('');
    setScopeInput('');
    const normalizedItemasignaturaName = normalizeasignaturaName(item.Asignatura?.nombre);
    const isProgrammingItem = normalizedItemasignaturaName.includes('programacion');
  ...
  const handleUpdateStudentPublication = async (item: MiniproyectoItem, visibleParaEstudiantes: boolean) => {
    ...
    try {
      ...
      }, docenteAsignaturaId ?? item.Asignatura?.id ?? null);

      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.message || errorData.error || 'No fue posible actualizar la asignación del miniproyecto del asignatura.');
      }
  ...
  const openCreateModal = () => {
    ...
    setCreateForm({
      ...
      asignaturaId: availableAsignaturas.length === 1 ? String(availableAsignaturas[0].id) : '',
    });
  ...
  const handleCreateConfigurableMiniproyecto = async (event: React.FormEvent) => {
    ...
    if (!createForm.asignaturaId) {
      setCreateError('Debes seleccionar un asignatura.');
      return;
    }
    ...
    try {
      const payload = {
        ...
        asignatura_id: Number(createForm.asignaturaId),
      ...
      }, Number(createForm.asignaturaId));
  ...
  useEffect(() => {
    ...
    setCostRows((prev) => {
      ...
      return deliverables.map((deliverable, index) => {
        const existing = prev[index] ?? prev.find((row) => normalizeasignaturaName(row.deliverable) === normalizeasignaturaName(deliverable));
  ...
  const selectedAsignaturaFilterName = availableAsignaturas.find((Asignatura) => String(Asignatura.id) === selectedAsignaturaFilterId)?.nombre || 'Todas las asignaturas';
  const selectedMiniproyectoTitle = selected?.Actividad?.titulo || 'Sin selección';
  const selectedMiniproyectoAsignatura = selected?.Asignatura?.nombre || 'Asignatura no definida';
  const asignaturasRepresented = groupedMiniproyectos.length;
              ...
              <div className="app-toolbar-card app-miniproyecto-toolbar app-miniproyecto-toolbar--hero">
                ...
                <div className="mb-3">
                  ...
                  <select
                    ...
                    disabled={isDocenteMode && Boolean(docenteAsignaturaId)}
                  >
                    {!isDocenteMode ? <option value="">Todas las asignaturas</option> : null}
            ...
            {isLoading ? (
              ...
              <div className="app-miniproyecto-group-stack">
                {groupedMiniproyectos.map((group, groupIndex) => (
                  <section key={group.Asignatura?.id || `group-${groupIndex}`} className="app-soft-card app-miniproyecto-Asignatura-group">
                    {(() => {
                      ...
                      return (
                        <>
                    {!isDocenteMode ? (
                      <div className="app-miniproyecto-Asignatura-group__header">
                        <div>
                          <p className="app-miniproyecto-Asignatura-group__eyebrow">Asignatura académica</p>
                          <h3 className="app-miniproyecto-Asignatura-group__title">{group.Asignatura?.nombre || 'Sin asignatura asignada'}</h3>
                        </div>
                        <div className="app-miniproyecto-Asignatura-group__meta">{group.items.length} miniproyecto(s)</div>
                      </div>
                    ) : null}

                    {!hasAssignedMiniproyecto ? (
                      <div className="app-miniproyecto-Asignatura-group__notice" role="status">
                        <span className="app-miniproyecto-Asignatura-group__notice-chip">No hay miniproyectos del asignatura</span>
                        <p className="app-miniproyecto-Asignatura-group__notice-text">
                          Esta asignatura todavía no tiene un miniproyecto asignado para estudiantes.
                        </p>
                      </div>
                    ) : null}
                    ...
                    <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="rounded-full bg-blue-100 px-3 py-1 text-[11px] font-semibold text-blue-700">
                          Asignatura: {selected?.Asignatura?.nombre || 'Sin asignatura'}
                        </span>
                  ...
                  >
                    ...
                    {isPublishing
                      ...
                      : selected?.seleccionadoParaEstudiantes
                        ? 'Quitar como miniproyecto del asignatura'
                        : 'Asignar como miniproyecto del asignatura'}
                  </button>
                  <button
                    type="submit"
                    disabled={isSaving}
