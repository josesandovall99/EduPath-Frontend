...
export function ConfigurableEmbeddedExerciseEditor({ exercises, onChange }: ConfigurableEmbeddedExerciseEditorProps) {
  ...
  const handleAddOrderingItem = (index: number, items: string[]) => {
    handleUpdateConfig(index, { items: [...items, `Item ${items.length + 1}`] });
  };

  const handleRemoveOrderingItem = (index: number, items: string[], itemIndex: number) => {
    handleUpdateConfig(index, { items: items.filter((_, currentIndex) => currentIndex !== itemIndex) });
  };

  const handleAddMatchingPair = (index: number, pairs: Array<{ concepto: string; definicion: string }>) => {
    handleUpdateConfig(index, { pares: [...pairs, { concepto: '', definicion: '' }] });
  };

  const handleRemoveMatchingPair = (index: number, pairs: Array<{ concepto: string; definicion: string }>, pairIndex: number) => {
    handleUpdateConfig(index, { pares: pairs.filter((_, currentIndex) => currentIndex !== pairIndex) });
  };
    ...
    <div className="space-y-4 rounded-[24px] border border-slate-200 bg-white p-4 shadow-[0_16px_36px_rgba(58,74,91,0.08)]">
      <div className="flex flex-col gap-3 rounded-[20px] border border-slate-200 bg-slate-50 p-4 md:flex-row md:items-center md:justify-between">
        <div className="space-y-1">
          ...
          <p className="max-w-2xl text-xs leading-5 text-slate-500">Configura la secuencia del estudiante en un panel compacto y edita un ejercicio a la vez.</p>
        </div>
      ...
      <div className="rounded-[20px] border border-slate-200 bg-white p-3 shadow-sm">
        <div className="mb-2 flex items-center justify-between gap-3">
          <div>
            ...
            <div className="text-xs text-slate-500">Crea un bloque nuevo con la plantilla del tipo elegido.</div>
          </div>
        </div>
        ...
        {EXERCISE_TYPES.map((type) => (
          <button
            ...
            className={`inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-xs font-medium text-[#243447] transition-all hover:-translate-y-0.5 ${EXERCISE_TYPE_META[type].soft}`}
          >
      ...
      {exercises.length === 0 ? (
        ...
        <div className="grid gap-4 lg:grid-cols-[216px_minmax(0,1fr)] xl:grid-cols-[224px_minmax(0,1fr)]">
          <aside className="rounded-[20px] border border-slate-200 bg-white p-3 shadow-sm lg:sticky lg:top-4 lg:h-fit">
            <div className="mb-2">
              <div className="text-sm font-semibold text-[#243447]">Mapa del miniproyecto</div>
              <div className="text-xs leading-5 text-slate-500">Selecciona una tarjeta para editar sus campos con más espacio.</div>
            </div>
              ...
              {exercises.map((exercise, index) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full rounded-xl border px-3 py-2.5 text-left transition-all ${isActive ? 'border-sky-300 bg-sky-50 shadow-sm' : 'border-slate-200 bg-white hover:border-slate-300 hover:bg-slate-50'}`}
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-semibold text-slate-500">{exercise.puntos}</span>
                    </div>
                  </button>
                );
              })}
          ...
          <div className="space-y-4">
            {selectedExercise ? (() => {
              ...
              const compilerConfig = exercise.tipo_ejercicio === 'Compilador'
                ? { ...createDefaultCompilerConfig(), ...(exercise.configuracion || {}) }
                : null;
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <div className="mt-1.5 flex flex-wrap gap-2">
                          <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-medium text-slate-600">{exercise.tipo_ejercicio}</span>
                          <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-medium text-slate-600">{exercise.puntos} puntos</span>
                        </div>
                      </div>
                    </div>
                    <button
                      ...
                      className="inline-flex items-center justify-center gap-2 rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-xs font-medium text-red-700 transition-colors hover:bg-red-100"
                    >
                  ...
                  <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(event) => handleUpdateExercise(index, { titulo: event.target.value })}
                        className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm focus:border-sky-300 focus:outline-none focus:ring-2 focus:ring-sky-100"
                      />
                    </div>
                    <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-3">
                      <div className="mb-1.5 flex items-center justify-between gap-3">
                        ...
                        <button
                          type="button"
                          onClick={() => setEditingTypeExerciseId((current) => current === exercise.id ? null : exercise.id)}
                      ...
                      <p className="mt-2 text-xs leading-5 text-slate-500">
                        El tipo ya quedó definido al agregar el ejercicio. Solo cámbialo si realmente quieres reemplazar la plantilla base.
                      </p>
                      {editingTypeExerciseId === exercise.id ? (
                        <select
                          value={exercise.tipo_ejercicio}
                          onChange={(event) => handleChangeExerciseType(index, event.target.value as ConfigurableExerciseType)}
                          className="mt-3 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm focus:border-sky-300 focus:outline-none focus:ring-2 focus:ring-sky-100"
                        >
                          {EXERCISE_TYPES.map((type) => (
                    ...
                    <div>
                      ...
                      <input
                        type="number"
                        min={1}
                        value={exercise.puntos}
                        onChange={(event) => handleUpdateExercise(index, { puntos: Number(event.target.value) || 1 })}
                        className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm focus:border-sky-300 focus:outline-none focus:ring-2 focus:ring-sky-100"
                      />
                    </div>
                    <div className="md:col-span-2 xl:col-span-3 rounded-xl border border-slate-200 bg-slate-50 p-3">
                      ...
                      <textarea
                        ...
                        onChange={(event) => handleUpdateExercise(index, { descripcion: event.target.value })}
                        rows={2}
                        className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm focus:border-sky-300 focus:outline-none focus:ring-2 focus:ring-sky-100"
                      />
                    </div>
                  </div>
                ...
                <div className="mt-4 space-y-4">
                      ...
                      <div className="rounded-[20px] border border-sky-100 bg-sky-50 p-4">
                        <div className="mb-3 flex items-center justify-between gap-3">
                          <div>
                            ...
                            <p className="mt-1 text-xs text-slate-500">Escribe el método que el estudiante deberá completar.</p>
                          </div>
                          <button
                            type="button"
                            onClick={() => handleUpdateExercise(index, { codigoEstructura: formatJavaLikeTemplate(methodTemplate) })}
                        ...
                        <textarea
                          ...
                          rows={10}
                          className="w-full rounded-[18px] border border-sky-100 bg-white px-4 py-3 font-mono text-sm leading-6 focus:border-sky-300 focus:outline-none focus:ring-2 focus:ring-sky-100"
                        />
                        <p className="mt-3 text-xs leading-5 text-slate-500">Debe ser un método Java válido. Se evaluará con 3 casos de prueba y no necesita clase `Main`.</p>
                      </div>

                      <div className="space-y-3">
                        <div className="rounded-[20px] border border-slate-200 bg-slate-50 px-4 py-3 text-xs leading-6 text-slate-600">
                          Método derivado: {derivedMethod ? `${derivedMethod.retorno} ${derivedMethod.nombre}(${derivedMethod.parametros.map((param) => `${param.tipo} ${param.nombre}`).join(', ')})` : 'Firma pendiente de detectar'}
                        </div>
                        ...
                        {(compilerCases.length ? compilerCases : createDefaultCompilerConfig().casos_prueba).map((caseItem, caseIndex) => (
                          <div key={`${exercise.id}-case-${caseIndex}`} className="rounded-[18px] border border-slate-200 bg-white p-3 shadow-sm">
                            ...
                            <input
                              ...
                              onChange={(event) => {
                                const nextCases = (compilerCases.length ? compilerCases : createDefaultCompilerConfig().casos_prueba).map((item, itemIndex) => (
                                  itemIndex === caseIndex ? { ...item, inputs: event.target.value } : item
                                ));
                                handleUpdateConfig(index, { casos_prueba: nextCases });
                              }}
                              placeholder="Inputs"
                              className="mb-2 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm focus:border-sky-300 focus:outline-none focus:ring-2 focus:ring-sky-100"
                            />
                            <input
                              value={caseItem.output || ''}
                              onChange={(event) => {
                                const nextCases = (compilerCases.length ? compilerCases : createDefaultCompilerConfig().casos_prueba).map((item, itemIndex) => (
                                  itemIndex === caseIndex ? { ...item, output: event.target.value } : item
                                ));
                                handleUpdateExercise(index, {
                                  resultado_ejercicio: nextCases[0]?.output || '',
                                  configuracion: {
                                    ...compilerConfig,
                                    casos_prueba: nextCases,
                                  },
                                });
                              }}
                              placeholder="Output esperado"
                              className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm focus:border-sky-300 focus:outline-none focus:ring-2 focus:ring-sky-100"
                            />
                          </div>
                        ))}
                      </div>
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                      <label className="rounded-[22px] border border-slate-200 bg-slate-50 p-4 text-sm text-[#243447]">
                        ...
                        <input
                          type="number"
                          min={1}
                          value={exercise.configuracion?.opciones?.minClasses ?? 2}
                          onChange={(event) => handleUpdateConfig(index, {
                            opciones: {
                              ...(exercise.configuracion?.opciones || {}),
                              minClasses: Number(event.target.value) || 1,
                            }
                          })}
                          className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 focus:border-sky-300 focus:outline-none focus:ring-2 focus:ring-sky-100"
                        />
                      </label>
                      <label className="flex items-center gap-3 rounded-[22px] border border-slate-200 bg-slate-50 p-4 text-sm text-[#243447]">
                        <input
                          type="checkbox"
                          checked={Boolean(exercise.configuracion?.opciones?.requireRelationships)}
                          onChange={(event) => handleUpdateConfig(index, {
                            opciones: {
                              ...(exercise.configuracion?.opciones || {}),
                              requireRelationships: event.target.checked,
                            }
                          })}
                          className="h-4 w-4"
                        />
                      ...
                      <label className="flex items-center gap-3 rounded-[22px] border border-slate-200 bg-slate-50 p-4 text-sm text-[#243447]">
                        <input
                          type="checkbox"
                          checked={Boolean(exercise.configuracion?.opciones?.requireMultiplicities)}
                          onChange={(event) => handleUpdateConfig(index, {
                            opciones: {
                              ...(exercise.configuracion?.opciones || {}),
                              requireMultiplicities: event.target.checked,
                            }
                          })}
                          className="h-4 w-4"
                        />
                  ...
                  {exercise.tipo_ejercicio === 'Opción única' ? (
                    <div className="space-y-3 rounded-[24px] border border-slate-200 bg-slate-50 p-4 md:p-5">
                      <input
                        value={exercise.configuracion?.enunciado || ''}
                        onChange={(event) => handleUpdateConfig(index, { enunciado: event.target.value })}
                        placeholder="Enunciado"
                        className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm focus:border-sky-300 focus:outline-none focus:ring-2 focus:ring-sky-100"
                      />
                      ...
                      <select
                        value={exercise.configuracion?.respuestaCorrecta || ''}
                        onChange={(event) => handleUpdateExercise(index, {
                          resultado_ejercicio: event.target.value,
                          configuracion: {
                    ...
                    <div className="space-y-3 rounded-[24px] border border-slate-200 bg-slate-50 p-4 md:p-5">
                      ...
                      {orderingItems.map((item, itemIndex) => (
                        <div key={`${exercise.id}-item-${itemIndex}`} className="flex items-center gap-2">
                          ...
                          <button
                            type="button"
                            onClick={() => handleRemoveOrderingItem(index, orderingItems, itemIndex)}
                            className="inline-flex h-11 w-11 items-center justify-center rounded-2xl border border-red-200 bg-red-50 text-red-700 transition-colors hover:bg-red-100"
                      ...
                      <button
                        type="button"
                        onClick={() => handleAddOrderingItem(index, orderingItems)}
                        className="inline-flex items-center gap-2 rounded-2xl border border-cyan-200 bg-cyan-50 px-4 py-3 text-sm font-medium text-cyan-700 transition-colors hover:bg-cyan-100"
                      >
                        <Plus className="h-4 w-4" />
                        Agregar ítem
                      </button>
                  ...
                  {exercise.tipo_ejercicio === 'Relacionar' ? (
                    <div className="space-y-3 rounded-[24px] border border-slate-200 bg-slate-50 p-4 md:p-5">
                      ...
                      {matchingPairs.map((pair, pairIndex) => (
                        <div key={`${exercise.id}-pair-${pairIndex}`} className="grid gap-3 rounded-2xl border border-slate-200 bg-white p-3 md:grid-cols-2">
                          ...
                          <div className="md:col-span-2 flex justify-end">
                            <button
                              type="button"
                              onClick={() => handleRemoveMatchingPair(index, matchingPairs, pairIndex)}
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-2 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-medium text-rose-700 transition-colors hover:bg-rose-100"
                      >
                        <Plus className="h-4 w-4" />
                        Agregar par
                      </button>
                        ...
                        <div key={question.id} className="rounded-[22px] border border-slate-200 bg-white p-4 shadow-sm">
                          <div className="mb-3 flex flex-col gap-3 md:flex-row">
                            <input
                              ...
                              className="flex-1 rounded-xl border border-slate-200 px-3 py-2.5 text-sm focus:border-sky-300 focus:outline-none focus:ring-2 focus:ring-sky-100"
                            />
                            <select
                              value={question.tipo}
                              onChange={(event) => {
                                const nextQuestions = questions.map((currentQuestion, currentIndex) => currentIndex === questionIndex ? {
                                  ...currentQuestion,
                                  tipo: event.target.value as EmbeddedQuestion['tipo'],
                                  opciones: event.target.value === 'abierta' ? [] : (currentQuestion.opciones && currentQuestion.opciones.length > 0 ? currentQuestion.opciones : ['Opción 1', 'Opción 2']),
                                  respuesta_correcta: event.target.value === 'abierta' ? '' : (currentQuestion.opciones?.[0] || ''),
                                } : currentQuestion);
                                handleUpdateConfig(index, { preguntas: nextQuestions });
                              }}
                              className="rounded-xl border border-slate-200 px-3 py-2.5 text-sm focus:border-sky-300 focus:outline-none focus:ring-2 focus:ring-sky-100"
                            >
                          ...
                          {question.tipo === 'opcion-multiple' ? (
                            <div className="space-y-2">
                                ...
                                <input
                                  ...
                                  onChange={(event) => {
                                    const nextQuestions = questions.map((currentQuestion, currentIndex) => {
                                      if (currentIndex !== questionIndex) return currentQuestion;
                                      const nextOptions = (currentQuestion.opciones || []).map((currentOption, currentOptionIndex) => currentOptionIndex === optionIndex ? event.target.value : currentOption);
                                      return {
                                        ...currentQuestion,
                                        opciones: nextOptions,
                                        respuesta_correcta: currentQuestion.respuesta_correcta === option ? event.target.value : currentQuestion.respuesta_correcta,
                                      };
                                    });
                                    handleUpdateConfig(index, { preguntas: nextQuestions });
                                  }}
                              ...
                              <select
                                ...
                                onChange={(event) => {
                                  const nextQuestions = questions.map((currentQuestion, currentIndex) => currentIndex === questionIndex ? { ...currentQuestion, respuesta_correcta: event.target.value } : currentQuestion);
                                  handleUpdateConfig(index, { preguntas: nextQuestions });
                                }}
                                className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm focus:border-sky-300 focus:outline-none focus:ring-2 focus:ring-sky-100"
                              >
                                <option value="">Respuesta correcta</option>
                                {(question.opciones || []).filter(Boolean).map((option) => (
                                  <option key={option} value={option}>{option}</option>
                                ))}
                              </select>
                            </div>
