...
// -- Offsets MVC (cuntas lneas hay ANTES de cada seccin en el merged file) --
...
/** Lneas ANTES del cuerpo de Main en el merged file. */
...
/** Lneas ANTES del cuerpo de Modelo en el merged file. */
...
function resolveMvcErrorSource(
  ...
  if (lineMatch) {
    ...
    if (mergedLine <= mainOffset) {
      // La lnea cae antes del Main ? pertenece al Modelo
...
/** Wrapper de DiagnosticBlock que resuelve automticamente la fuente (Main vs Modelo). */
...
function parseCompilerError(error: string, offset = 0): {
  ...
  // Extrae la lnea de cdigo y el caret que muestra el compilador
...
function buildFriendlyCompilerAction(rawError: string, parsed: {
  ...
  // Heurstica: cuando Java reporta "';' expected" pero realmente hay una cadena/carcter suelto en la lnea actual o anterior.
  if (errorTypeLower.includes("';' expected") || errorLower.includes("';' expected")) {
    if (isLonelyToken(currentTrim) || isLonelyToken(previousTrim)) {
      ...
      return `Hay una cadena/carcter suelto ("${token}") que rompe la sintaxis. Si fue accidental, elimnalo; si era una variable, sala dentro de una instruccin vlida.`;
    }

    // Si no est en la lnea reportada, buscar hacia arriba (hasta 5 lneas),
    ...
    if (studentLine && studentLine > 1) {
      for (let i = studentLine - 2; i >= Math.max(0, studentLine - 6); i -= 1) {
        ...
        if (isLonelyToken(candidate)) {
          return `Hay una cadena/carcter suelto ("${candidate}") que rompe la sintaxis. Si fue accidental, elimnalo; si era una variable, sala dentro de una instruccin vlida.`;
        }
        // Si encontramos una lnea de cdigo real no-token, detenemos la bsqueda.
        break;
      }
    }
  }
  ...
  return fallback || parsed.errorType || 'Revisa la firma del mtodo o campo referenciado.';
}
...
function DiagnosticBlock({ error, className = '', studentCode, offset = 0 }: { error: string; className?: string; studentCode?: string; offset?: number }) {
  ...
  // Columna relativa del ^ respecto al contenido trimmeado de la lnea de error
  let caretRelCol = -1;
  ...
  return (
    <div className={`overflow-hidden rounded-xl border border-rose-200 bg-white shadow-[0_4px_14px_rgba(244,63,94,0.10)] ${className}`}>
      {/* Header  banda roja con cono + estado + lnea */}
      <div className="flex items-center justify-between gap-3 border-b border-rose-200/70 bg-gradient-to-r from-rose-50 to-rose-50/40 px-4 py-3">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-sm font-semibold text-rose-700">Error de compilacin</span>
        </div>
      ...
      {/* Accin sugerida  footer con cono lightbulb, sin caja amarilla suelta */}
      <div className="flex items-start gap-2.5 px-4 py-3 bg-amber-50/40">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-amber-700 mb-1">Accin sugerida</p>
...
// -- Proteccin de estructura del docente ------------------------------------

/** Verifica que TODAS las lneas no triviales del template del docente sigan presentes. */
/**
 * Simula el echo del terminal: inserta los valores de entrada del caso
 * despus de cada lnea de prompt (lneas que terminan en ':').
...
/**
 * LCS order check  las lneas del docente deben aparecer EN ORDEN en el cdigo.
 * Protege lneas duplicadas (ej: dos '}') y evita reordenamientos o eliminaciones.
 */
function isStructureIntact(currentCode: string, templateCode: string): boolean {
  ...
  const required = templateCode
    .split('\n')
    .map((l) => l.trim())
    .filter((l) => l.length > 0 && !/^\/\/\s*(Tu\s+c[o]digo|implementa|agrega|aqu[i]|here|TODO|FIXME|\.\.\.|your\s+code)/i.test(l));
...
function getCaseMessage(caso: CasoPruebaResultado) {
  ...
  if (caso.omitido) return 'La revisin se detuvo  un caso anterior no cumpli.';
  if (caso.paso) return 'La solucin cumpli correctamente con este caso.';
  if (isCompilerError(caso.error)) return 'Error de compilacin  revisa el diagnstico.';
  if (caso.outputObtenido) return 'La salida obtenida no coincide con el resultado esperado.';
  return 'La solucin no produjo el resultado esperado para este caso.';
}
...
export function ProgrammingContentView({ content, onBack, onComplete, embedded = false, configurableMode = false, configurableResponse, onConfigurableResponseChange, configurableApproved = false, exerciseId, exerciseData = null, executePath, submitPath }: ProgrammingContentViewProps) {
  ...
  const guideItems = [
    'Firma inalterarable: la estructura base del ejercicio (clases y mtodos definidos por el docente) no se puede borrar ni modificar. Solo tienes permiso para editar o eliminar el cdigo que t mismo hayas escrito dentro de esa estructura.',
    'Archivos permitidos: realiza modificaciones exclusivamente en los archivos indicados en el enunciado del ejercicio.',
    'ConsolaIO.java es el ncleo que gestiona la comunicacin con el evaluador. Es de solo lectura  consltala para entender el flujo de entrada/Salida del compilador.',
    'Salida del compilador: toda referencia a resultados de ejecucin se muestra bajo esa etiqueta. Usa Ejecutar para validar tu lgica y Enviar para la evaluacin final.',
    'Si el cdigo falla, revisa el diagnstico: [ESTADO]  [LNEA]  [ACCIN]  aplica la correccin indicada antes de volver a Ejecutar.'
  ];
  ...
  // Offset dinmico: cuntas lneas hay ANTES del cuerpo de Main en el archivo MVC fusionado
  ...
  const handleExecute = async () => {
    ...
    if (isMvcMode || rawCfgGuard.tipo === 'mvc') {
      if (rawCfgGuard.templateMain && !isStructureIntact(mvcMainCode, rawCfgGuard.templateMain)) {
        toast.error('[ESTADO]: Estructura comprometida  Main.java', { description: '[ACCIN]: La firma del docente fue modificada. Usa "Restaurar plantilla".', duration: 6000 });
      ...
      if (rawCfgGuard.templateModelo && !isStructureIntact(mvcModeloCode, rawCfgGuard.templateModelo)) {
        toast.error(`[ESTADO]: Estructura comprometida  ${mvcNombreModelo}.java`, { description: '[ACCIN]: La firma del docente fue modificada. Usa "Restaurar plantilla".', duration: 6000 });
        setIsRunning(false); setResultMode('idle'); return;
      }
    } else {
      const tpl = getInitialTemplate(ejercicio);
      if (tpl && !isStructureIntact(code, tpl)) {
        toast.error('[ESTADO]: Estructura comprometida', { description: '[ACCIN]: La firma del docente fue modificada. Usa "Restaurar plantilla".', duration: 6000 });
        setIsRunning(false); setResultMode('idle'); return;
      }
    }
    // -----------------------------------------------------------------------
    ...
    if (data?.debug) {
      console.log('DEBUG ejecucin compilador:', data.debug);
      toast.info('Debug: revisa la consola (Network ? Response) para ms detalles');
    }
    const casosList = Array.isArray(data?.casos) ? data.casos
      : Array.isArray(data?.casosPrueba) ? data.casosPrueba : [];

    // 200 = todos los casos pasaron | 400 con casos = algunos fallaron (ambos son resultados vlidos)
    if (result.status === 200 || (result.status === 400 && casosList.length > 0)) {
      setCasosPruebaResultados(casosList);
      const resumen = data?.resumen || (result.status === 200 ? 'Todos los casos pasaron.' : 'Algunos casos no pasaron. Revisa tu cdigo.');
      setFeedback(resumen);
      if (result.status === 200) {
        toast.success('Ejecucin completada', { description: resumen });
      } else {
        toast.info('Ejecucin completada', { description: resumen });
      }
      setIsRunning(false);
      return;
    }

    // Error tcnico real (500, red error, etc.)
    const errMsg = data?.message || result.message || 'No fue posible ejecutar la solucin.';
    setFeedback(errMsg);
    setCasosPruebaResultados(casosList);
    ...
    <div className={wrapperClassName}>
      ...
      <div className={workspaceClassName}>
        ...
        <section className={`${cardClass} relative overflow-hidden`}>
          ...
          {/*
            La descripcin se almacena como HTML enriquecido (Quill).
            ...
            seguro inyectarlo aqu. La clase `html-content` aporta estilos
            tipogrficos consistentes definidos en `index.html`.
          */}
          {ejercicio?.actividad?.descripcion ? (
            ...
            <p className="mt-4 border-t border-slate-100 pt-4 text-[14px] leading-6 text-gray-600">Sin descripcin disponible.</p>
          )}
        </section>
        ...
        <section className="grid grid-cols-1 gap-5">
          <div className={`${cardClass} relative flex min-w-0 flex-col overflow-hidden`}>
            ...
            <div className="mb-4 border-b border-slate-100 pb-3">
              <span className={sectionLabelClass}>Gua</span>
              ...
              {configurableMode ? (
                configurableApproved ? (
                  ...
                  <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm text-slate-600">
                    La solucin actual se evaluar con el botn principal del miniproyecto.
                  </div>
                )
              ) : (
                <button
          ...
          <div className="flex flex-col lg:min-h-0 lg:flex-1">
            {isMvcMode ? (
              <div className="flex flex-col" style={{ minHeight: 'calc(100vh - 320px)' }}>
                ...
                {/* -- Barra de pestaas MVC  mismo gradiente que el toolbar -- */}
                ...
                {/* -- Asignatura Monaco  altura fija en px para que Monaco pueda calcularla -- */}
                <div
                  style={{
                ...
                <div className="shrink-0 border-t border-slate-200 bg-white p-5">
                  ...
                  {!aprobado && syntaxValidationErrors.length > 0 && (
                    <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm">
                      ...
                      <div className="mb-2 text-amber-700">Ajusta tu solucin para incluir estas estructuras obligatorias:</div>
                  ...
                  {casosPruebaResultados.length === 0 ? (
                    <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50 px-4 py-6 text-sm text-slate-500">
                      Presiona <strong>Ejecutar</strong> para probar tu cdigo contra los casos del docente, o <strong>Enviar</strong> para calificar.
                    </div>
                  ) : (
                  ...
                  <textarea
                    ...
                    onChange={(e) => {
                      ...
                      if (tpl && !isStructureIntact(next, tpl)) {
                        toast.error('[ESTADO]: Lnea protegida', {
                          description: '[ACCIN]: Esta lnea es parte de la estructura del docente y no puede eliminarse.',
                          duration: 2500,
                        });
            ...
            {!isMvcMode && (
              ...
              {isLoadingExercise ? (
                ...
                <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50 px-4 py-6 text-sm text-slate-500">
                  {resultMode === 'execution' ? 'La ejecucin mostrar la salida por caso.' : 'El envo mostrar el estado de cada caso.'}
                </div>
              ) : (
