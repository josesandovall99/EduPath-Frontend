    ...
    <div className="space-y-4 p-4 bg-yellow-50 rounded-lg border border-yellow-200">
      ...
      <div>
        ...
        <input
          ...
          placeholder="Describe la tAsignatura a ordenar"
...
export function ExerciseManagementScreen({
  ...
  type AsignaturaOption = { id: number; nombre: string; estado?: boolean };

  type TemaOption = { id: number; nombre: string; asignatura_id: number; estado?: boolean };
  ...
  const [asignaturasOptions, setAsignaturasOptions] = useState<AsignaturaOption[]>([]);
  ...
  const [contentFilters, setContentFilters] = useState({
    ...
    asignaturaId: docenteAsignaturaId ? String(docenteAsignaturaId) : '',
  ...
  // ── Filtros de lista por jerarquía académica ─────────────────────────────
  // Para el docente, el filtro de asignatura queda pre-fijado y deshabilitado.
  ...
  const getAuthHeaders = () => {
    ...
    return {
      ...
      ...(isDocenteMode && docenteAsignaturaId ? { 'x-asignatura-id': String(docenteAsignaturaId) } : {})
  ...
  useEffect(() => {
    ...
    // Sincroniza también el filtro de lista con la asignatura del docente.
    if (docenteAsignaturaId) setFilterAsignaturaId(String(docenteAsignaturaId));
  ...
  const loadContenidos = async () => {
    ...
    try {
      ...
      const [contenidosRes, asignaturasRes, temasRes, subtemasRes] = await Promise.all([
        ...
        fetch(`${API_BASE_URL}${isDocenteMode ? '/asignaturas/mis-asignaturas' : '/asignaturas'}`, {
      ...
      const [data, asignaturasCatalog, temasCatalog, subtemasCatalog] = await Promise.all([

        contenidosRes.json(),

        asignaturasRes.ok ? asignaturasRes.json() : Promise.resolve([]),
      ...
      const asignaturasMapped: AsignaturaOption[] = (asignaturasCatalog || [])

        .filter((Asignatura: any) => Asignatura?.estado !== false)

        .map((Asignatura: any) => ({

          id: Number(Asignatura.id),

          nombre: Asignatura.nombre || `Asignatura ${Asignatura.id}`,

          estado: Asignatura.estado

        }))

        .filter((Asignatura: AsignaturaOption) => Number.isFinite(Asignatura.id));
      ...
      const temasMapped: TemaOption[] = (temasCatalog || [])
        ...
        .map((tema: any) => ({
          ...
          asignatura_id: Number(tema.asignatura_id),

          estado: tema.estado

        }))

        .filter((tema: TemaOption) => Number.isFinite(tema.id) && Number.isFinite(tema.asignatura_id));
      ...
      const asignaturasById = new Map(asignaturasMapped.map((Asignatura) => [Number(Asignatura.id), Asignatura]));
      ...
      const mapped: ContenidoOption[] = (data || []).map((c: any) => {
        ...
        const asignaturaId = Number(c.asignatura_id ?? c.Asignatura?.id ?? c.Asignatura?.id ?? tema?.asignatura_id);

        const Asignatura = Number.isFinite(asignaturaId) ? asignaturasById.get(asignaturaId) : undefined;
        ...
        return {
          ...
          asignatura_id: Number.isFinite(asignaturaId) ? asignaturaId : undefined,

          tema_id: Number.isFinite(temaId) ? temaId : undefined,

          subtema_id: Number.isFinite(subtemaId) ? subtemaId : undefined,

          asignaturaNombre: Asignatura?.nombre || c.Asignatura?.nombre || c.Asignatura?.nombre,
      ...
      setAsignaturasOptions(asignaturasMapped);
  ...
  const filteredEjercicios = useMemo(() => {
    // Mapa contenido_id → ContenidoOption para resolver asignatura/tema/subtema del ejercicio.
    const contenidoMap = new Map(contenidosOptions.map((c) => [c.id, c]));
  ...
  const openEdit = (item: EjercicioItem) => {
    ...
    setContentFilters({
      ...
      asignaturaId: isDocenteMode && docenteAsignaturaId

        ? String(docenteAsignaturaId)

        : (contenidoActual?.asignatura_id ? String(contenidoActual.asignatura_id) : ''),
  ...
  const activeAsignaturaFilter = isDocenteMode && docenteAsignaturaId ? String(docenteAsignaturaId) : contentFilters.asignaturaId;

  const temaFilterOptions = temasOptions.filter((tema) => (

    !activeAsignaturaFilter || Number(tema.asignatura_id) === Number(activeAsignaturaFilter)
  ...
  const contentMatchesFilters = (contenido: ContenidoOption, filters = contentFilters) => {
    ...
    const asignaturaId = isDocenteMode && docenteAsignaturaId ? String(docenteAsignaturaId) : filters.asignaturaId;



    if (asignaturaId && Number(contenido.asignatura_id) !== Number(asignaturaId)) return false;
  ...
  const handleContentFilterChange = (field: 'asignaturaId' | 'temaId' | 'subtemaId', value: string) => {
    ...
    const nextFilters = {
      ...
      ...(field === 'asignaturaId' ? { temaId: '', subtemaId: '' } : {}),
              ...
              <div className="app-form-layout app-form-layout--with-aside lg:px-8 lg:py-7">
                ...
                <form id="exercise-form" onSubmit={handleSubmit} className="app-form-main app-form-stack">
                  ...
                  <section className="app-form-section">
                    ...
                    {/* Filtros de contenido — columna única para ver texto completo */}
                    <div className="app-form-grid">
                      ...
                      {!isDocenteMode && (
                        ...
                        <div className="app-form-field">
                          ...
                          <label className="app-form-label">Filtrar por asignatura</label>

                          <select

                            value={contentFilters.asignaturaId}

                            onChange={(event) => handleContentFilterChange('asignaturaId', event.target.value)}
                          ...
                          >
                            ...
                            {asignaturasOptions.map((Asignatura) => (

                              <option key={Asignatura.id} value={String(Asignatura.id)}>
                      ...
                      <div className="app-form-field">
                        ...
                        <select
                          ...
                          disabled={isLoadingContenidos || (isDocenteMode && !activeAsignaturaFilter)}
